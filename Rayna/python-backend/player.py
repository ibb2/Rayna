...
class AudioPlayer:
    ...
    def seek(self, position_seconds: float):
        # Not fully implemented for streaming, but placeholder
        pass
        ...
        try:
            ...
            def callback(outdata, frames, time, status):
                ...
                if remaining < chunk_size:
                    ...
                    outdata[:] = self.current_data[self.position:self.position + chunk_size]
                    self.position += chunk_size

            with sd.OutputStream(samplerate=samplerate, channels=data_array.shape[1] if len(data_array.shape) > 1 else 1, callback=callback):
                while self.position < len(self.current_data) and not self.stop_event.is_set():
                    if not self.is_playing:
    ...
    def get_status(self):
        return {
            ...
            "position": self.position / self.current_samplerate if self.current_samplerate else 0,
            "duration": len(self.current_data) / self.current_samplerate if self.current_data is not None else 0
        }
